<template>
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="flex justify-between items-center mb-8">
        <h1 class="text-2xl font-bold text-[#611232]">Registro de Criterios de Evaluación</h1>
        <button 
          @click="regresar"
  ...
  const cargarCriterios = () => {
    ...
    // Filtrar criterios existentes para el concurso seleccionado
    const criteriosDelConcurso = props.criteriosExistentes.filter(
      c => c.concurso_id == concursoId.value
    );
    
    // Si hay criterios existentes, usarlos; de lo contrario, empezar con uno vacío
  ...
  const selectMenu = (item) => {
    selectedMenu.value = item.name.toLowerCase(); // Asegúrate de que el nombre coincida con el bloque en Concursos.vue
